...
class MIRA:
    def __init__(self, port):
        ...
        print("MIRA sensor is loaded.")
        self.readData = b'\x24\x3E\x00\x41\x0D'
        initData = [0x24, 0x3E, 0x00, 0x42, 0x0D]
        self.ser.write(initData)

    def get_value(self):
        self.ser.write(self.readData)
        value = self.ser.read().hex()
        value = float.fromhex(''.join(value))
        return value
